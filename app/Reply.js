  ...
  render(){
    ...
    return (
      <section className="reply">
        <img src={this.props.state.profile.avatar} className="reply-avatar" />
        <div className="reply-body">
          <textarea
            ...
            className={this.props.state.reply.focus ? 'reply-selected' : ''}
            value={this.props.state.reply.message}
        ...
        <div className={this.props.state.reply.focus ? 'reply-controls' : 'hidden'}>
          ...
          <input href="#" className={this.props.state.reply.message.length===0 ? 'btn pull-left disabled' : 'btn pull-left'} accept=".gif,.jpg,.jpeg,.png" type="file" onChange={this.imageSelect.bind(this)} id={"reply-camara-" + randomID}>
          </input>

          <span ref="charCounter" className="char-counter">{config.tweets.maxTweetSize - this.props.state.reply.message.length }</span>

          <button className={this.props.state.reply.message.length===0 ? 'btn btn-primary disabled' : 'btn btn-primary '} onClick={this.newTweet.bind(this)}>
...
export default connect( mapStateToProps,{updateReplyForm, resetReplyForm} )(Reply);
